...
- Материал предоставляет: ${data.materialOwner === 'client' ? 'заказчик' : data.materialOwner === 'contractor' ? 'исполнитель (нужно купить)' : 'не указано'}
...
4. ВАЖНО — учёт стоимости материала:
   - Если материал предоставляет ЗАКАЗЧИК (materialOwner = "client"): стоимость материала НЕ включать в aiMin/aiMax, считай только работу и операции.
   - Если материал покупает ИСПОЛНИТЕЛЬ (materialOwner = "contractor"): обязательно заложи стоимость материала в aiMin/aiMax:
     • Оцени примерный объём материала по заявке (толщина, длина швов, описание);
     • Добавь к стоимости работ цену материала (листы/профиль), расходники (электроды/проволока, газ), логистику;
     • Такие работы дороже минимум в 1.5–2 раза относительно аналогичных работ с материалом заказчика;
     • ОБЯЗАТЕЛЬНО учитывай наценку на материал для покрытия рисков, хранения и доставки.

5. Обязательно учитывай налог, расходные материалы, амортизацию оборудования, напарника и нормальную прибыль мастера: итоговый диапазон aiMin/aiMax не должен превращать работу в убыток.

6. Очень важно: используй следующий референс-кейс для калибровки цен.
   Пример: латунные полосы, толщина 3 мм, длина швов около 1700 см, полный цикл работ (сварка, зачистка, шлифовка, сатинирование, прорисовка полос, покраска, покрытие лаком), материал покупает ИСПОЛНИТЕЛЬ — в нормальных условиях мастерской стоит порядка 200 000–300 000 ₽. Любая похожая или более сложная работа не может стоить дешевле 200 000 ₽.

7. В aiMin и aiMax всегда давай честный диапазон, а не одну точку: обычно разброс 15–30 % от средней оценки.

8. ЗАПРЕЩЕНО слепо копировать входные значения localMin/localMax. Ты должен сам рассчитать итоговую стоимость на основе сложности, материала, объёма и того, кто покупает материал.

Сформируй reasonShort как короткий комментарий, почему цена получилась такой (материал, длина швов, количество операций, вредность, доступ, кто покупает материал и т.п.). Если материал покупает исполнитель, обязательно упомяни это.
...
- ЕСЛИ материал покупает исполнитель (materialOwner = "contractor"): отдельным предложением объясни, что в цену входит закупка материала (листы/профиль), расходные материалы (электроды, проволока, газ), логистика, поэтому цена выше. Подчеркни, что это избавляет клиента от необходимости самому искать и покупать материал.
