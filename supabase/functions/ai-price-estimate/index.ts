...
ВАЖНО: Верни ТОЛЬКО JSON без текста до и после, без \`\`\`:
{
  "aiMin": число (минимальная цена),
  "aiMax": число (максимальная цена),
  "proposal": "текст коммерческого предложения клиенту на русском языке, 3-5 абзацев",
  "warnings": ["предупреждения, если нужны"]
}`;
