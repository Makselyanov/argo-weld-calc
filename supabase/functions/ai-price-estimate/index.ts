...
- Предварительный расчёт (localMin): ${data.localMin}
- Предварительный расчёт (localMax): ${data.localMax}
- Описание: ${data.description || "нет"}

ЗАДАНИЕ:

Ты должен использовать уже рассчитанные значения цены (localMin и localMax) как финальные.
Твоя задача — НЕ пересчитывать цену, а грамотно обосновать её клиенту.

finalMin = ${data.localMin}
finalMax = ${data.localMax}

ПРАВИЛА:
1. aiMin и aiMax в ответе ДОЛЖНЫ быть равны ${data.localMin} и ${data.localMax}. Не придумывай свои числа.
2. Сфокусируйся на описании сложности работы:
   - Если это латунь/медь: токсичность, сложность сварки, риск дефектов.
   - Если длинные швы: большой объём кропотливой работы.
   - Если полный цикл: зачистка, шлифовка, покраска — это много ручного труда.
3. Если materialOwner = "contractor", обязательно укажи, что цена включает стоимость материала, расходников и логистику.

Сформируй reasonShort как короткий комментарий, почему цена получилась такой (материал, длина швов, количество операций, вредность, доступ, кто покупает материал и т.п.).
...
aiMin и aiMax — целые числа в рублях РФ, aiMin < aiMax. Если не можешь оценить, верни aiMin = 0, aiMax = 0 и текст-объяснение в warnings.`;
        ...
        return new Response(
            JSON.stringify({
                aiMin: parsed.aiMin ?? data.localMin,
                aiMax: parsed.aiMax ?? data.localMax,
                aiFailed: false,
