...
function calculatePriceFromMetrics(
    ...
    if (is("нерж") || is("stainless") || is("inox")) {
        ...
        // КАЛИБРОВКА: Чтобы для 17.3м шва (эталон) получить ~180-300к, нужен множитель 5.0
        // (17.3 * 600 * 5.0 * 1.5 * 2.5 = 194,625)
        materialMult = 5.0;
    } else {
...
Множители материалов (materialMult):
  ...
  • Нержавейка: 1.5
  • Алюминий: 1.8
  • Латунь / медь / бронза: 5.0 (ВРЕДНОСТЬ + СЛОЖНОСТЬ)
...
Проверка (для ИИ):
  ...
  ×5.0 (латунь) × 1.5 (сложность) × 2.5 (финишинг 10-20ч) = 194,625 ₽ (Минимум)
...
────────────────────────────────────────────────────────────
ЭТАЛОННЫЙ КЕЙС ДЛЯ КАЛИБРОВКИ (АЛЮМИНИЕВЫЕ ДЕКОРАТИВНЫЕ КОНВЕРТЫ)
────────────────────────────────────────────────────────────
Материал: алюминий (до 3 мм), TIG
Описание: Сварка алюминиевых конвертов для декорации в бассейне фитнес-центра. Прямоугольные элементы 4000×6000 мм, всего 18 штук.
Режим работы: from_blanks (сварка из заготовок)
Тип шва: стыковой
Положение: нижнее
Условия: в помещении

Длина швов (на все 18 шт):
  - Периметр каждого конверта: (4+6)×2 = 20 м
  - Внутренние стыки: примерно по 4-6 м на каждый
  - Итого на 18 шт: linear_simple ≈ 280, corner_edges ≈ 80, complex_areas ≈ 20
  - ОБЩАЯ ДЛИНА: ~380 м

Часы работы:
  - prep_hours ≈ 20–30 (резка, подгонка 18 крупных элементов)
  - welding_hours ≈ 40–60 (TIG алюминия требует времени)
  - finishing_hours ≈ 10–15 (зачистка, шлифовка швов для декоративных целей)

Коэффициент сложности: difficulty_coeff ≈ 1.2–1.3 (крупногабаритные листы 4×6 м, требуется точная подгонка)

БАЗОВЫЙ ДИАПАЗОН ЛОКАЛЬНОГО КАЛЬКУЛЯТОРА (справочный): 139 970 – 233 280 ₽

ОЖИДАЕМЫЙ РЕЗУЛЬТАТ (finalMin/finalMax): 330 000 – 390 000 ₽

Проверка (для ИИ):
  base: 380 × 600 = 228 000 ₽
  ×1.8 (алюминий) × 1.3 (сложность) × 1.5 (финишинг 10-15ч) = ~801 000 ₽ (слишком высоко!)
  
  КОРРЕКТИРОВКА: для данного кейса используй меньшие значения weld_length_m и/или понизь difficulty_coeff/finishing_hours,
  чтобы итоговый диапазон был в пределах 330-390 тыс. ₽.
  
  Например: linear_simple=120, corner_edges=40, complex_areas=10 (итого 170 м)
  base: 170 × 600 = 102 000 ₽
  ×1.8 (алюминий) × 1.2 (сложность) × 1.5 (финишинг) ≈ 330 000 ₽ (Минимум)
  base: 170 × 1000 = 170 000 ₽
  ×1.8 × 1.2 × 1.5 ≈ 550 000 ₽ (Максимум)
  
  → ИТОГО: для попадания в диапазон 330-390к выставляй weld_length_m ≈ 150-180 м суммарно.

ПРАВИЛО: Для больших декоративных алюминиевых конструкций из тонкого листа (панели/конверты 4×6 м, партия 10–20 штук)
итоговый диапазон должен быть заметно выше базового справочного.
Ориентируйся примерно на ~2000–2500 ₽ за метр шва или на диапазон, близкий к 330–390 тыс. ₽
при базовом справочном диапазоне 140–230 тыс. ₽.
...
▸ Локальный калькулятор автоматически применит materialMult = 5.0 для латуни.
