...
function generateProposal(
    ...
    // Определяем, является ли материал цветным металлом (используется в нескольких блоках)
    const isColoredMetal =
        materialLower.includes("латун") ||
        materialLower.includes("медь") ||
        materialLower.includes("бронз") ||
...
▸ Базовая ставка за метр латунного шва (чистая сварка с подготовкой и зачисткой) составляет примерно 700–1 200 ₽/м (внутренняя оценка для калькулятора).
...
▸ Если в описании работ ОДНОВРЕМЕННО присутствуют:
  ...
  ТО:
   • используй общий коэффициент сложности в диапазоне 8–15 к чистой сварке;
   • ориентировочный диапазон по факту получается 12 000–18 000 ₽/м;
   • при длине швов около 17–18 м итоговая цена должна попадать примерно в диапазон 250 000–300 000 ₽.
